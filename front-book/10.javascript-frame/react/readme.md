...
应用级框架：每次更新流程都是从应用的根节点开始，遍历整个应用也就是从`<App/>` 开始向下更新
vue：更新流程开始于组件。
svelte 的更新流程开始与元素。
由于该特性，react 不需要细粒度更新和 AOT(Ahead Of Time： 提前编译或预编译)
react 能力
...
1. react 面对的是什么问题？如何从运行时找到解决方案
2. react 从 v15 到 v16 后，为什么要重构底层架构
3. 重构后新架构如何工作
4. 如何快速调试源码
...
1. CPU 瓶颈：执行大计算量的操作或者设备性能不足，页面调整卡顿
2. I/O 瓶颈：进行 I/O 操作后，需要等待数据返回才能操作，等待的过程导致不能快速响应

**事件循环：浏览器为什么会卡顿**
浏览器如何动起来的，主线程通过循环执行任务队列里的的事件
![alt text](assets/image-6.png)

因为事件循环机制，加入的任务队列的新任务需要等待队列中其他的任务都执行完后才能执行，这对用户交互过程中“突发情况下需要优先执行的任务”是不利的。

任务队列中的任务为宏任务，为了解决时效性，宏任务执行过过程中产生微任务，保证在该任务执行上下文中的微任务队列中。在宏任务执行结束前，线程遍历微任务队列，将宏任务产生的微任务批量执行。

例如 MutationObserver Api，同一个宏任务发生多次 DOM 变化，产生多个 mutationObserver 微任务，执行时机是该宏任务执行结束前，这样相比于作为新的宏任务进行队列等待执行，这种机制更能保证时效性。
...
5. Paint：绘制
   渲染流水线
   流水线并不一定全部执行
6. js 或 css 修改 dom 元素几何属性(长,宽)，会触发完整的渲染流水线，即重排
7. 修改属性不涉及几何属性比如字体颜色时，会省略 layout 和 layer，即重回
8. 当修改比如不涉及重排、重绘的属性（比如 transform 属性）会省略 layout、layer、paint，仅执行合成线程的绘制工作，即合成
   性能排序：重排<重绘<合成

## 2.2 底层架构演进

**CUP 瓶颈**
react 中造成 CPU 瓶颈是 VDOM 相关工作，例如渲染 3000 个 Li 元素，这个时候的 vdom 就要转为 dom，会有 cpu 瓶颈。
react 做法将 vdom 的执行过程拆分一个一个独立的宏任务，将每个宏任务的执行事件限制在一定范围类（5ms）这样就会使一个会造成掉帧的长任务拆解为多个不会造成掉帧的端宏任务，这就是 Time Slice（时间切片）
把「渲染 Virtual DOM 的过程」切成可中断的多个小任务（切片，slice
![alt text](assets/image-7.png)

**I/O 瓶颈**
减少网络延迟对用户的影响：

1. 为不同的操作造成的自变量变化赋予不同优先级
2. 所有优先级同一调度，有限处理最高优先级的更新
3. 如果更新正在进行（即进入 vdom 相关工作），有更高优先级的更新产生，则会中断当前更新，优先处理高优先级更新。
...
为什么 15 升级到 16，就是因为旧架构无法实现 time slice

react15 架构

- reconciler 协调器 - vdom 的实现，负责根据自变量变化计算出 UI 变化
- render 渲染器 - 负责将 ui 变化渲染的宿主环境中

reconciler 中，mount 的组件会调用 mountCoumponent，update 组件会调用 updatecomponent，这两个方法都会递归更新组件，更新流程一旦开始，中途无法中断。

react16 架构

- scheduler 调度器 - 调度任务的优先级，高优先级任务优先进入 reconciler
- reconciler 协调器 - vdom 的实现，负责更具自变量变化计算出 UI 变化
- render 渲染器 - 负责将 UI 变化渲染到宿主环境

reconciler 中的更新流程从递归变成了可中断的循环过程。每次循环都会调用 shouldyield 判断当前 timeslice 是否还有剩余时间。如果没有旧暂停新流程，将主线程交给渲染流水线，等待下一个宏任务再继续执行，这就是 time slice 实现原理

```
function workLoopConcurrent() {
  // 一直执行任务，直到任务执行完成中断
   while (wordInProgress !== null $$ shouldYield()) {
...
每个 time slice 宏任务的时间长度 5ms 左右。

reconciler 为 vdom 元素标记各种副作用 flags
// 代表插入或移动元素
export const placement = 0b0000000000000010
// 代表更新元素
export const update = 0b000000000000100
// 代表删除元素
export const deletion = ob00000000000001000
当 reconciler 完成工作后，工作流程才会进入 render。render 更具 reconciler 为 vdom 元素标记的各种 flags 执行对应的操作。
...
虚线框内的工作是在内存中进行的，不会更新宿主环境 UI，因此即使工作反复中断，用户也不会看到更新不完全的 ui
...
4. concurrent feature 并发特性
   CPU 瓶颈 reconciler 的工作流从同步 => 异步可中断 async mode
   单一更新不能完全突破 io 瓶颈，解决关键在于多个更新工作流程并发执行 concurrent mode 并发模式。使多个更新的工作流程可以并发执行。

## 2.3 Fiber 架构

Fiber VDOM 在 react 中的实现，新架构的基础。

react 中三种节点类型

1. react element react 元素，即 createElement 方法的返回值
2. react component react 组件，开发者可以在 react 定义函数、类两种类型的组件组件
3. fiberNode 组成 fiber 架构的节点内类型
...
1. v16 及以后版本 reconciler 基于 FiberNode 实现，称为 FiberReconciler
2. 作为静态的数据结构，每一个 FiberNode 对应一个 react 元素，用于保存 React 元素的类型、对应的 DOM 元素等信息。
3. 作为动态的工作单元，每个 fiberNode 用于保存 本次更新中该 react 元素变化的数据、要执行的工作（增，删，改，更新 ref，副作用等）
...
作为静态的数据结构，fiberNode 保存了从 react 元素获取到的信息

```javascript
// 对应组件的类型 function class host
this.tag = tag;
// key属性
this.key = key;
// 大部分情况同type，某些情况不同，比如functionComponent使用REact.memo包括
this.elementType = null;
// 对于functioncomponent指函数本省
// 对于class component 指class
// 对于hostComponent 指dom tagName小写形式，比如以下这些
// <div /> → hostComponent = 'div'
// <span /> → hostComponent = 'span'
// <input /> → hostComponent = 'input'
this.type = null;
// FiberNode对应的元素，比如functionComponent对应的DOM元素
this.stateNode = null;
```

作为动态的工作单元，fiberNode 保存了更新相关的信息
...
**双缓存机制**
Fiber 架构中同时存在两个 fiber tree，一颗是真实 UI 对应的 fiber tree,前缓冲区。
另一颗是正在内存中构建的 fiber tree，后缓冲区。宿主环境可以类比为显示器

源码中很多接受 current 和 workInProgress 作为参数，current 前缓冲区的 fiber node，workInProgress 后缓冲区中的 fiber node。alternate 属性指向另一个缓冲区中对应的 fiber node
...
1. 整个应用的首次渲染，这种情况发生在首次进入页面的时候
2. 某一个组件的首次渲染
   `isShow ? <Btn/> : null`
   mount 时 fiber tree 的构建过程如下
3. 创建 fiberRootNode
4. 常见 tag = 3 的 fiberNode，代表 hostRoot
5. 从 hostRootFiber 开始，以 DFS 深度优先搜索顺序产生 fiberNode
6. 便利过程中为 FiberNode 标记不同副作用的 flags，以便后续在 renderer 中使用
...
hostRoot 代表应用在宿主环境挂在的跟节点即 rootElement。HostRootFiber 代表 HostRoot 对应的 FiberNode
FiberRootNode 负责管理该应用的全局事宜，比如：

- current fiber tree 与 wip fiber tree 之间的钱换
- 应用中任务的过期时间
- 应用的任务调度信息

ReactDom.createRoot 会创建 fiber 结构
![alt text](assets/image-11.png)
fiberRootNode.current 指向 current fiber tree 的根节点，当前仅有一个 hostRootFiber，对应首屏渲染时仅有根节点的空白页面。
mount 流程会基于每个 react 元素以 DFS 的顺序依次生成 wip fiber node，并连接起来构成 wip fiber tree。如图 2-9 所示.

生成 wip fiberNode 时会复用 current fiber tree 中的同级节点，在 mount 流程中，只有 hostRootFIber 存在对应节点，由 alternate 链接，当 wip fiber tree 生成完毕后，fiberRootNode 会被传递给 renderer，更具过程中标记的副作用有关 flags 执行对应操作。

render 完成工作后，wip fiber tree 对应 ui 已经渲染到宿主环境中，fiberRootNode.current 指向 wip hostRootFiber 完成双缓存的切换工作，曾经的 wip fiber tree 变成 current fiber tree

![](assets/20251020_144517_image.png)

**update 时 fiber tree 的构建**
如果点击前面代码 p 元素触发更新，将会开启 update 流程，生成一课新的 wip fiber tree。
![alt text](assets/image-12.png)

fiber 的工作原理，开发者可以在一个页面创建多个应用，比如如下代码会在页面中创建 3 个应用。此时会存在 3 个 fiber root node
...
- react 包含全局 react api 的所有核心模块
- scheduler 调度器实现
- react-reconciler fiber 架构
...
根据 scheduler 的调度结果不同，render 阶段可能开始于 performSyncWorkOnRoot 同步更新流程或并发更新流程犯法
